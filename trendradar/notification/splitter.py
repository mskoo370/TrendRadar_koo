...
def split_content_into_batches(
    ...
    """分批处理消息内容，确保词组标题+至少第一개新闻的完整性（支持热榜+RSS合并+AI分析+독립 쇼케이스）

    热榜统计与RSS统计并列显示，热榜新增与RSS新增并列显示。
    region_order 控制各区域的显示顺序。
    AI分析内容根据 region_order 中的位置显示。
    독립 쇼케이스根据 region_order 中的位置显示。
    ...
    Args:
        ...
        get_time_func: 获取현재时间的函数（可选）
        rss_items: RSS 统计개目列表（按源分组，用于合并推送）
        ...
        standalone_data: 독립 쇼케이스数据（可选），包含 platforms 和 rss_feeds 列表
    ...
    if ai_stats and ai_stats.get("analyzed_news", 0) > 0:
        ...
        if format_type in ("wework", "bark", "ntfy", "feishu", "dingtalk"):
            ai_stats_line = f"**AI 분석 수：** {news_display}{mode_suffix}\n"
    ...
    if report_data["stats"]:
        if format_type in ("wework", "bark"):
            stats_header = f"📊 **{stats_title}** (총 {total_hotlist_count} 개)\n\n"
        elif format_type == "telegram":
            stats_header = f"📊 {stats_title} (총 {total_hotlist_count} 개)\n\n"
        elif format_type == "ntfy":
            stats_header = f"📊 **{stats_title}** (총 {total_hotlist_count} 개)\n\n"
    ...
    if (
        ...
        and not standalone_data  # 有독립 쇼케이스数据时也不返回
    ):
        if mode == "incremental":
    ...
    def process_new_titles_section(current_batch, current_batch_has_content, batches, add_separator=True):
        ...
        if add_separator and current_batch_has_content:
            # 需要添加分割线
            if format_type in ("wework", "bark"):
                new_header = f"\n\n\n\n🆕 **本次신규 핫 트렌드新闻** (총 {report_data['total_new_count']} 개)\n\n"
            elif format_type == "telegram":
                new_header = (
                    f"\n\n🆕 本次신규 핫 트렌드新闻 (총 {report_data['total_new_count']} 개)\n\n"
                )
            ...
            # 不需要分割线（第一个区域）
            if format_type in ("wework", "bark"):
                new_header = f"🆕 **本次신규 핫 트렌드新闻** (총 {report_data['total_new_count']} 개)\n\n"
            elif format_type == "telegram":
                new_header = f"🆕 本次신규 핫 트렌드新闻 (총 {report_data['total_new_count']} 개)\n\n"
            elif format_type == "ntfy":
                new_header = f"🆕 **本次신규 핫 트렌드新闻** (총 {report_data['total_new_count']} 개)\n\n"
...
def _process_rss_stats_section(
    ...
    Args:
        ...
        base_footer: 基础尾部
        max_bytes: 最大字节数
        current_batch: 현재批次内容
        current_batch_has_content: 현재批次是否有内容
        batches: 已完成的批次列表
        timezone: 时区名称
    ...
    if add_separator and current_batch_has_content:
        ...
        # 不需要分割线（第一个区域）
        if format_type == "feishu":
            rss_header = f"📰 **RSS 구독 통계** (총 {total_items} 개)\n\n"
        elif format_type == "dingtalk":
            rss_header = f"📰 **RSS 구독 통계** (총 {total_items} 개)\n\n"
        elif format_type == "telegram":
            rss_header = f"📰 RSS 구독 통계 (총 {total_items} 개)\n\n"
    ...
    for i, stat in enumerate(rss_stats):
        ...
        # 构建第一개新闻（使用 format_title_for_platform）
        first_news_line = ""
        if stat["titles"]:
            first_title_data = stat["titles"][0]
            if format_type in ("wework", "bark"):
...
def _process_rss_new_titles_section(
    ...
    if add_separator and current_batch_has_content:
        ...
        # 不需要分割线（第一个区域）
        if format_type in ("wework", "bark"):
            new_header = f"🆕 **RSS 本次新增** (총 {total_items} 개)\n\n"
        elif format_type == "telegram":
            new_header = f"🆕 RSS 本次新增 (총 {total_items} 개)\n\n"
        elif format_type == "ntfy":
            new_header = f"🆕 **RSS 本次新增** (총 {total_items} 개)\n\n"
    ...
    for i, (source_name, titles) in enumerate(source_list):
        ...
        source_header = ""
        if format_type in ("wework", "bark"):
            source_header = f"**{source_name}** ({count} 개):\n\n"
        elif format_type == "telegram":
            source_header = f"{source_name} ({count} 개):\n\n"
        elif format_type == "ntfy":
            source_header = f"**{source_name}** ({count} 개):\n\n"
...
def _process_standalone_section(
    ...
    독립 쇼케이스显示指定平台的完整热榜或 RSS 源内容，不受关键词过滤影响。
    ...
    # 독립 쇼케이스标题（根据 add_separator 决定是否添加前置分割线）
    section_header = ""
    if add_separator and current_batch_has_content:
        # 需要添加分割线
        if format_type == "feishu":
            section_header = f"\n{feishu_separator}\n\n📋 **독립 쇼케이스** (총 {total_items} 개)\n\n"
        elif format_type == "dingtalk":
            section_header = f"\n---\n\n📋 **독립 쇼케이스** (총 {total_items} 개)\n\n"
        elif format_type in ("wework", "bark"):
            section_header = f"\n\n\n\n📋 **독립 쇼케이스** (총 {total_items} 개)\n\n"
        ...
        # 不需要分割线（第一个区域）
        if format_type == "feishu":
            section_header = f"📋 **독립 쇼케이스** (총 {total_items} 개)\n\n"
        elif format_type == "dingtalk":
            section_header = f"📋 **독립 쇼케이스** (총 {total_items} 개)\n\n"
        elif format_type == "telegram":
            section_header = f"📋 독립 쇼케이스 (총 {total_items} 개)\n\n"
    ...
    for platform in platforms:
        ...
        platform_header = ""
        if format_type in ("wework", "bark"):
            platform_header = f"**{platform_name}** ({len(items)} 개):\n\n"
        elif format_type == "telegram":
            platform_header = f"{platform_name} ({len(items)} 개):\n\n"
        elif format_type == "ntfy":
            platform_header = f"**{platform_name}** ({len(items)} 개):\n\n"
    ...
    for feed in rss_feeds:
        ...
        feed_header = ""
        if format_type in ("wework", "bark"):
            feed_header = f"**{feed_name}** ({len(items)} 개):\n\n"
        elif format_type == "telegram":
            feed_header = f"{feed_name} ({len(items)} 개):\n\n"
        elif format_type == "ntfy":
            feed_header = f"**{feed_name}** ({len(items)} 개):\n\n"
...
def _format_standalone_platform_item(item: Dict, index: int, format_type: str, rank_threshold: int = 10) -> str:
    """格式化독립 쇼케이스的热榜개目（复用热点词汇统计区样式）

    Args:
        item: 热榜개目，包含 title, url, rank, ranks, first_time, last_time, count
        index: 序号
        format_type: 格式类型
        rank_threshold: 排名高亮阈值
...
def _format_standalone_rss_item(
    ...
    Args:
        item: RSS 개目，包含 title, url, published_at, author
        index: 序号
        format_type: 格式类型
        timezone: 时区名称
